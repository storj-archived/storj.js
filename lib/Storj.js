...
Storj.prototype.createFile = function(bucketId, fileName, opts, cb) {
  ...
  self._client.createToken(bucketId, 'PULL', function(err, token) {
    ...
    var blob = self._store();
    var storeStream = blob.createWriteStream({ key: tmpName })
...
Storj.prototype._storeFileInBucket = function() {
  ...
  var rs = blob.createReadStream(this._meta.tmpName);
 ...
 * errors occurs
 */
Storj.getFile = function(bucketId, fileId, callback) {
  // get file info 
  client.getFileInfo(bucketId, fileId, function(fileInfo){
