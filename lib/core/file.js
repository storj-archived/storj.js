 ...
 * @prop {Number} speed - File upload/download speed, in bytes/sec
 * @prop {Number} progress - File upload/download progress, from 0 to 1
 * @prop {Number} numPeers - Count of farmers we are actively connected to
...
function File(self) {
  ...
  this._encryptionKey = self._encryptionKey;
...
File.prototype.createFile = function(bucketId, fileName, stream) {
  ...
  self._client.createToken(bucketId, 'PUSH', function(err, token) {
    ...
    if(token.encryptionKey !== '') {
      self._encryptionKey = token.encryptionKey;
    } else {
      if(self._encryptionKey === undefined) {
    ...
    var fileKey = KeyIV.getDeterministicKey(self._encryptionKey, fileId);
...
File.prototype.getBuffer = function getBuffer(cb) {
  ...
  this._store.createReadStream(this._fileId).pipe(c).on('error', cb);
};
